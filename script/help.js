    ...
    try {
        ...
        if (!input || !isNaN(input)) {
            ...
            for (let i = start; i < end; i++) {
                helpMessage += `${i + 1}. ${prefix}${commands[i].config.name}\n`;
            }
            ...
            const command = commands.find(cmd => cmd.config.name.toLowerCase() === input.toLowerCase() || (cmd.config.aliases && cmd.config.aliases.includes(input.toLowerCase())));
